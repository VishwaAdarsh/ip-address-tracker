...
| Total Observations Collected | **2** | Unique Evaluated Domains |
| Quota Completion | **4.0%** | Progress Towards 50 Target |
| Remaining Observations Needed | **48** | Deficit to Target Quota |
| Valid Host Records | **2** | Fully Resolved IP Hosts |
| Data Quality Audit Status | **Clean (0 Warnings)** | Pre-Export Verification |

## 5. Transport Security & Cryptographic Posture
Analysis of the cohort reveals an HTTPS adoption rate of **50.0%** (1 of 2 endpoints). TLS certificate validation confirms that **50.0%** of endpoints presented valid, verifiable cryptographic trust chains.

| Transport Metric | Count | Percentage | Classification State |
|:---|:---:|:---:|:---|
| HTTPS Enabled | 1 | 50.0% | Encrypted Transport Active |
| HTTPS Disabled / Cleartext | 1 | 50.0% | Insecure HTTP Only |
| HTTPS State Unknown | 0 | 0.0% | Unreachable / Blocked |
| TLS Certificate Valid | 1 | 50.0% | Valid Chain of Trust |
| TLS Invalid / Expired | 1 | 50.0% | Security Degradation Flag |
...
Cohort Website Trust Scores demonstrated a central mean of **50.0 / 100** (Min: 5.0, Max: 95.0, StdDev: 45.0). Conversely, IP Risk Scores demonstrated a mean of **12.5 / 100** (Min: 10.0, Max: 15.0, StdDev: 2.5).
...
Infrastructure categorization indicates that **100.0%** (2 endpoints) are situated within hyper-scale Cloud, Datacenter, or CDN networks (e.g., Cloudflare, AWS, Google Cloud, Akamai), while **0** endpoints operate on traditional enterprise or telecom carrier transit lines.
...
- **Sample Quota Progress** (`REPUTATION`): Field study in progress: 2 of 50 target observations recorded (48 remaining to fulfill quota). *(Metric: 2 / 50 (4.0%))*
- **Transport Layer HTTPS Adoption** (`SECURITY`): 1 of 2 valid observed websites (50.0%) operate with active HTTPS transport encryption. *(Metric: 50.0% Encrypted)*
- **Cloud & Datacenter Concentration** (`INFRASTRUCTURE`): 2 of 2 observations (100.0%) were classified as large-scale Cloud or Datacenter infrastructure. *(Metric: 100.0% Cloud / DC)*
- **Cohort Mean Scores** (`REPUTATION`): Cohort mean Website Trust Score is 50.0/100, paired with an average IP Risk Score of 12.5/100. *(Metric: Trust 50.0/100 | Risk 12.5/100)*
...
- **Leading Autonomous System** (`NETWORK`): Network routing was led by AS15169 Google LLC (Google LLC), routing 1 endpoints (50.0% of cohort). *(Metric: AS15169 Google LLC (50.0%))*
...
**Sample Collection:** 2 of 50 target observations recorded.  
**Target Quota Status:** `INCOMPLETE (2/50)`  
